...
export function useAvifConversionQueue({ settings, conversionEnabled, onNotify }) {
  ...
  const start = useCallback(async () => {
    ...
    if (!conversionEnabled) {
      notify(ERROR_CODES.AVIF_LOAD_FAILED);
      return;
    }
    ...
    try {
      while (true) {
        ...
        const result = await convertImageToAvif(nextItem.file, settings, {
