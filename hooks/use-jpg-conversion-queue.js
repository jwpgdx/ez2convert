...
export function useJpgConversionQueue({ settings, conversionEnabled, onNotify }) {
  ...
  const start = useCallback(async () => {
    ...
    if (!conversionEnabled) {
      notify(ERROR_CODES.JPG_ENCODE_UNSUPPORTED);
      return;
    }
    ...
    try {
      while (true) {
        ...
        const result = await convertImageToJpg(nextItem.file, settings, {
