...
import { encodeAvifImageData } from "../engines/image-avif/avif-engine";
...
function normalizeAvifTune(value) {
  if (value === "ssim") {
    return "ssim";
  }
  return "auto";
...
function normalizeAvifChromaSubsampling(value) {
  if (value === "4:2:0" || value === "4:4:4") {
    return value;
  }
  return "auto";
}

function normalizeAvifConversionOptions(options = {}) {
  const base = normalizeConversionOptions({
    ...options,
    quality: options.quality ?? DEFAULT_AVIF_QUALITY,
  });
  ...
  return {
    ...base,
    avifOptions: {
      tune: normalizeAvifTune(avifOptions.tune),
      chromaSubsampling: normalizeAvifChromaSubsampling(
        avifOptions.chromaSubsampling
      ),
    },
  };
}
...
function mapAvifErrorCode(error, signal) {
  if (signal?.aborted) {
...
export async function convertImageToAvif(file, options = {}, execution = {}) {
  const signal = execution.signal;
  const normalizedOptions = normalizeAvifConversionOptions(options);
  ...
  try {
    ...
    const avifBytes = await encodeAvifImageData({
    ...
    const blob = new Blob([avifBytes], { type: MIME_TYPES.AVIF });
    if (!blob.size) {
      throw createEngineError(ERROR_CODES.AVIF_ENCODE_FAILED);
    }
    ...
    return {
      ...
      output: {
        blob,
        url,
        name: `${getBaseName(file.name)}.avif`,
        size: blob.size,
    ...
    return {
      ok: false,
      errorCode: mapAvifErrorCode(error, signal),
    };
  } finally {
    decoded?.cleanup?.();
    if (orientedCanvas) {
