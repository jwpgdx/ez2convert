...
import { encodeVideoToGif } from "../engines/video-gif/ffmpeg-engine";
import { ERROR_CODES } from "../constants/error-codes";
import { MIME_TYPES, isVideoMimeType } from "../config/file-types";
...
function mapVideoErrorCode(error, signal) {
  ...
  if (
    error?.message === ERROR_CODES.USER_CANCELED ||
    error?.code === ERROR_CODES.USER_CANCELED
  ) {
    return ERROR_CODES.USER_CANCELED;
  }
  ...
  if (
    ...
    error?.code === ERROR_CODES.VIDEO_GIF_ENCODE_FAILED ||
  ...
  return mapUnknownErrorCode(error, ERROR_CODES.VIDEO_GIF_ENCODE_FAILED);
}

export async function convertVideoToGif(file, options = {}, execution = {}) {
  ...
  try {
    ...
    const outputBytes = await encodeVideoToGif({
    ...
    const blob = new Blob([outputBytes], { type: MIME_TYPES.GIF });
