...
import {
  normalizeJpgConversionOptions,
  resolveResizeDimensions,
} from "../engines/common/resize";
import { applyAlphaBackground } from "../engines/common/alpha-flatten";
import {
...
import { encodeJpegImageData } from "../engines/image-jpg/jpeg-engine";
import { encodeCanvasToJpegBlob } from "../engines/image-jpg/canvas-jpeg-engine";
...
function mapJpgErrorCode(error, signal) {
  if (signal?.aborted) {
...
async function encodeWithFallback({ canvas, imageData, options, signal }) {
  try {
    const jpegBytes = await encodeJpegImageData({
      imageData,
      options,
      signal,
    });

    const blob = new Blob([jpegBytes], { type: MIME_TYPES.JPEG });
    if (!blob.size) {
      throw createEngineError(ERROR_CODES.JPEG_ENCODE_FAILED);
    }
    ...
    if (
      error?.code !== ERROR_CODES.JPEG_LOAD_FAILED &&
      error?.code !== ERROR_CODES.JPEG_ENCODE_FAILED
    ) {
      throw error;
    }

    const fallbackBlob = await encodeCanvasToJpegBlob({
      canvas,
    ...
    return {
      blob: fallbackBlob,
      engineUsed: "canvas-fallback",
    };
  }
}

export async function convertImageToJpg(file, options = {}, execution = {}) {
  const signal = execution.signal;
  const normalizedOptions = normalizeJpgConversionOptions(options);
  ...
  try {
    ...
    applyAlphaBackground(resizedCanvas, normalizedOptions.alphaBackground);
    const sourceImageData = canvasToImageData(resizedCanvas);

    const { blob, engineUsed } = await encodeWithFallback({
    ...
    return {
      ok: false,
      errorCode: mapJpgErrorCode(error, signal),
    };
  } finally {
    decoded?.cleanup?.();
    if (orientedCanvas) {
