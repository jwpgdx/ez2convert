...
import { encodeAnimatedToGif } from "../engines/animated-gif/ffmpeg-engine";
...
function mapAnimatedErrorCode(error, signal) {
  ...
  if (
    ...
    error?.code === ERROR_CODES.ANIMATED_GIF_ENCODE_FAILED
  ) {
    return error.code;
  }
  ...
  return mapUnknownErrorCode(error, ERROR_CODES.ANIMATED_GIF_ENCODE_FAILED);
}

export async function convertAnimatedToGif(file, options = {}, execution = {}) {
  ...
  try {
    ...
    const outputBytes = await encodeAnimatedToGif({
