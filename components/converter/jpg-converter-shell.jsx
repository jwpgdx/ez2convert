...
import { useJpgConversionQueue } from "@/hooks/use-jpg-conversion-queue";
import {
  SETTINGS_DEFAULTS,
  flattenSettingsToConversionOptions,
} from "@/lib/converters/constants";
import { getErrorMessage } from "@/lib/converters/error-messages";
import { checkJpgEncodeSupport } from "@/lib/converters/jpg-encode-support";
import {
...
const ZIP_NAME_PREFIX = "jpg-converted";

let toastIdSeed = 0;

export default function JpgConverterShell() {
  ...
  const queue = useJpgConversionQueue({
  ...
  useEffect(() => {
    ...
    async function runSupportCheck() {
      const supported = await checkJpgEncodeSupport();
      if (!isMounted) {
        return;
      }
      setSupportStatus(
  ...
  useEffect(() => {
    if (supportStatus === SUPPORT_STATUS.UNSUPPORTED) {
      addToast({
        code: ERROR_CODES.JPG_ENCODE_UNSUPPORTED,
        message: getErrorMessage(ERROR_CODES.JPG_ENCODE_UNSUPPORTED),
      });
    }
